...
class Product extends \Eccube\Entity\AbstractEntity
{
    ...
    /*
      2016/04/16 金子修正
      EC-CUBE本体のバグだったのでプラグイン側ではなく本体を修正
    */
    public function _calc()
    {
        if (!$this->_calc) {
            ...
            foreach ($this->getProductClasses() as $ProductClass) {
                ...
                if ($ProductClass->getClassCategory1()) {
                    ...
                    if (!empty($classCategoryId1)) {
                        $tempclassCategories1[$ProductClass->getClassCategory1()->getRank()]['id'] = $ProductClass->getClassCategory1()->getId();
                        $tempclassCategories1[$ProductClass->getClassCategory1()->getRank()]['name'] = $ProductClass->getClassCategory1()->getName();
                        if ($ProductClass->getClassCategory2()) {
                            $tempclassCategories2[$ProductClass->getClassCategory2()->getRank()]['id'] = $ProductClass->getClassCategory2()->getId();
                            $tempclassCategories2[$ProductClass->getClassCategory2()->getRank()]['name'] = $ProductClass->getClassCategory2()->getName();
                        }
                    }
                }
                $i++;
            ...
            foreach($tempclassCategories1 as $category1){
                $this->classCategories1[$category1['id']] = $category1['name'];
            }
            krsort($tempclassCategories2);
            foreach($tempclassCategories2 as $category1){
                $this->classCategories2[$category1['id']] = $category1['name'];
            }
            $this->_calc = true;
        }
    }
