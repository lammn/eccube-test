...
class ProductClassController
{
    ...
    {
        $tempClassCategories1 = array();
        if ($ClassName1) {
            $tempClassCategories1 = $app['eccube.repository.class_category']->findBy(array('ClassName' => $ClassName1));
        }
        $tempClassCategories2 = array();
        if ($ClassName2) {
            $tempClassCategories2 = $app['eccube.repository.class_category']->findBy(array('ClassName' => $ClassName2));
        }

        $tempclassCategories1 = array();
        $tempclassCategories2 = array();
        foreach($tempClassCategories1 as $ClassCategory1){
            if ($ClassCategory1) {
                $tempclassCategories1[$ClassCategory1->getRank()] = $ClassCategory1;
            }
        }
        krsort($tempclassCategories1);
        foreach($tempclassCategories1 as $category1){
            $ClassCategories1[] = $category1;
        }
        foreach($tempClassCategories2 as $ClassCategory2){
            if ($ClassCategory2) {
                $tempclassCategories2[$ClassCategory2->getRank()] = $ClassCategory2;
            }
        }
        krsort($tempclassCategories2);
        foreach($tempclassCategories2 as $category2){
            $ClassCategories2[] = $category2;
        }
    ...
    private function getProductClassesExcludeNonClass(Product $Product)
    {
        ...
        if ($ProductClass->getClassCategory1()) {
            ...
            if (!empty($classCategoryId1)) {
                $tempclassCategories1[$ProductClass->getClassCategory1()->getId()]['id'] = $ProductClass->getClassCategory1()->getId();
                $tempclassCategories1[$ProductClass->getClassCategory1()->getId()]['name'] = $ProductClass->getClassCategory1()->getName();
                if ($ProductClass->getClassCategory2()) {
                    $tempclassCategories2[$ProductClass->getClassCategory1()->getId()]['id'] = $ProductClass->getClassCategory2()->getId();
                    $tempclassCategories2[$ProductClass->getClassCategory1()->getId()]['name'] = $ProductClass->getClassCategory2()->getName();
                }
            }
        }
        krsort($tempclassCategories1);
        foreach($tempclassCategories1 as $category1){
            $ClassCategory1[$category1['id']] = $category1['name'];
        }
        krsort($tempclassCategories1);
        foreach($tempclassCategories2 as $category1){
            $ClassCategory2[$category1['id']] = $category1['name'];
        }
