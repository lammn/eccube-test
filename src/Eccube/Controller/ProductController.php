...
use Eccube\Entity\ProductRequest;
...
class ProductController
{
    ...
    public function index(Application $app, Request $request)
    {
        ...
        if($request->query->get('pricerange') && $request->query->get('pricerange') == '1'){
            $qb->andWhere('pc.price02 BETWEEN 0 AND 5000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '2'){
            $qb->andWhere('pc.price02 BETWEEN 5000 AND 10000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '3'){
            $qb->andWhere('pc.price02 BETWEEN 10000 AND 20000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '4'){
            $qb->andWhere('pc.price02 BETWEEN 20000 AND 30000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '5'){
            $qb->andWhere('pc.price02 BETWEEN 30000 AND 40000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '6'){
            $qb->andWhere('pc.price02 BETWEEN 40000 AND 50000');
        }else if($request->query->get('pricerange') && $request->query->get('pricerange') == '7'){
            $qb->andWhere('pc.price02 > 50000');
        }
    ...
    public function detail(Application $app, Request $request, $id)
    {
        ...
        if ($request->getMethod() === 'POST') {
            ...
            if ($form->isValid()) {
                ...
                if ($addCartData['mode'] === 'item_request') {
                    if ($app->isGranted('ROLE_USER')) {
                        $ProductRequest = new ProductRequest();
                        $ProductRequest->setProductId($Product->getId());
                        $ProductRequest->setUserId($app->user()->getId());
                        $app['orm.em']->persist($ProductRequest);
                        $app['orm.em']->flush($ProductRequest);
                        $app['session']->getFlashBag()->set('product_detail.just_added_request', $Product->getId());
                        return $app->redirect($app->url('product_detail', array('id' => $Product->getId())));
                    } else {
                        // 非会員の場合、ログイン画面を表示
                        //  ログイン後の画面遷移先を設定
                        $app->setLoginTargetPath($app->url('product_detail', array('id' => $Product->getId())));
                        $app['session']->getFlashBag()->set('eccube.add.product_request', true);
                        return $app->redirect($app->url('mypage_login'));
                    }
                }
            ...
            //product request
            $addRequest = $app['session']->getFlashBag()->get('eccube.add.product_request');
            if (!empty($addRequest)) {
                // お気に入り登録時にログインされていない場合、ログイン後にお気に入り追加処理を行う
                if ($app->isGranted('ROLE_USER')) {
                    $ProductRequest = new ProductRequest();
        ...
        if ($app->isGranted('ROLE_USER')) {
//            $ProductRequest = $app['eccube.repository.product_request']->findOneBy(array(
...
//                $is_product_request = true;
        ...
        $p_name = str_replace("　", "<br />", $Product['name']);
    ...
    public function request(Application $app, Request $request, $id)
    {
